...
def train_surprise_model(n_epochs=20, verbose=True):
    ...
    export_model(
        model, model_name, model_type="surprise", model_ext="pkl", verbose=True
    )


def export_model(
    model, model_name="model", model_type="surprise", model_ext="pkl", verbose=True
):
...
def load_model(model_type="surprise", model_ext="pkl"):
    path_latest = settings.MEDIA_ROOT / \
        f"ml/models/{model_type}/latest.{model_ext}"
    model = None
